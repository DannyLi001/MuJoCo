...
class RobotController:
    ...
    def __init__(self, model: mujoco.MjModel, data: mujoco.MjData, 
        ...
        self.end_effector_id = mujoco.mj_name2id(
            model, mujoco.mjtObj.mjOBJ_BODY, end_effector_name
        )
        ...
        self.kp_impedance = np.array([300, 300, 300, 50, 50, 50])  # 阻抗刚度 (位置+姿态)
        self.kd_impedance = np.array([30, 30, 30, 5, 5, 5])        # 阻抗阻尼
    
    # ========== 位置控制器 ==========
    def position_control(self, target_pos: np.ndarray, 
                        target_rot: Optional[np.ndarray] = None,
                        kp: Optional[float] = None,
                        kd: Optional[float] = None) -> np.ndarray:
        """
        位置级控制: PD控制器在笛卡尔空间
    ...
    # ========== 速度控制器 ==========
    def velocity_control(self, target_vel: np.ndarray,
                        kp: Optional[float] = None) -> np.ndarray:
        """
        速度级控制: 跟踪笛卡尔空间速度
    ...
    # ========== 加速度控制器 ==========
    def acceleration_control(self, target_pos: np.ndarray,
                           target_vel: np.ndarray,
                           target_acc: np.ndarray,
                           target_rot: Optional[np.ndarray] = None,
                           kp: Optional[float] = None,
                           kd: Optional[float] = None) -> Tuple[np.ndarray, Dict]:
        """
        加速度级控制: 使用操作空间动力学控制
    ...
    def impedance_control(self, target_pos: np.ndarray,
                         ...
                         damping: Optional[np.ndarray] = None) -> np.ndarray:
        """
